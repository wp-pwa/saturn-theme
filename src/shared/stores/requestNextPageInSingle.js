...
export default self => ({
      ...
      if (column.hasExtracted('horizontal')) {
        ...
        if (!item.list.page(item.page).isReady && !item.list.page(item.page).isFetching) {
          self.connection.fetchListPage({
            list: { type: item.type, id: item.id, page: item.page },
          });
  ...
  requestNextPageInSingle: flow(function* requestNextPageInSingle() {
    ...
    if (!nextPage.isReady && !nextPage.isFetching)
      self.connection.fetchListPage({ list: { type, id, page } });
    ...
    if (!connection.selectedContext.hasItem({ item }))
      self.connection.addColumnToContext({ column: [{ type, id, page, extract: 'horizontal' }] });
    // Wait until it's ready.
    yield when(
      () =>
        connection.list(type, id).page(page).isReady || connection.selectedContext.index !== index,
    );
  }),
});
