...
export const getEntity = createSelector(state => state, getWpType, getId, (state, wpType, id) =>
  dep('connection', 'selectorCreators', 'getWpTypeById')(wpType, id)(state),
);

export const getEntityMedia = createSelector(getEntity, entity => entity.featured_media);

export const getEntityTitle = createSelector(getEntity, entity => entity.title.rendered);

export const getEntityLink = createSelector(getEntity, entity => entity.link);

export const areCurrentCountsReady = createSelector(state => state, getId, (state, id) =>
  selectorCreators.shareModal.areCountsReady(id)(state),
);
...
export const getCurrentTotalCounts = createSelector(
  ...
  (state, id, countsReady) =>
    countsReady ? selectorCreators.shareModal.getTotalCounts(id)(state) : 0,
);
